...
MACRO(LINK_INTERNAL TRGTNAME)
    ...
    ELSE(NOT CMAKE24)
        FOREACH(LINKLIB ${ARGN})
            IF(MSVC AND OpenPointCloud_MSVC_VERSIONED_DLL)
...
MACRO(LINK_CORELIB_DEFAULT CORELIB_NAME)
    ...
    IF (OpenPointCloud_GLES1_AVAILABLE OR OpenPointCloud_GLES2_AVAILABLE)
        SET(ALL_GL_LIBRARIES ${ALL_GL_LIBRARIES} ${OPENGL_egl_LIBRARY})
    ENDIF()

    LINK_EXTERNAL(${CORELIB_NAME} ${ALL_GL_LIBRARIES}) 
    #LINK_WITH_VARIABLES(${CORELIB_NAME} OPENTHREADS_LIBRARY)
    IF(OpenPointCloud_SONAMES)
      SET_TARGET_PROPERTIES(${CORELIB_NAME} PROPERTIES VERSION ${OpenPointCloud_VERSION} SOVERSION ${OpenPointCloud_SOVERSION})
    ENDIF(OpenPointCloud_SONAMES)
...
MACRO(SETUP_LIBRARY LIB_NAME)
    ...
    ELSE()
        ...
        ADD_LIBRARY(${LIB_NAME}
            ${OpenPointCloud_USER_DEFINED_DYNAMIC_OR_STATIC}
            ${TARGET_H}
...
MACRO(SETUP_PLUGIN PLUGIN_NAME)
    ...
    SET(TARGET_SRC ${TARGET_SRC} ${OpenPointCloud_VERSIONINFO_RC})
    
    # here we use the command to generate the library    

    IF   (DYNAMIC_OpenPointCloud)
        ADD_LIBRARY(${TARGET_TARGETNAME} MODULE ${TARGET_SRC} ${TARGET_H})
    ELSE (DYNAMIC_OpenPointCloud)
        ADD_LIBRARY(${TARGET_TARGETNAME} STATIC ${TARGET_SRC} ${TARGET_H})
    ENDIF(DYNAMIC_OpenPointCloud)
    ...
    ELSE(NOT MSVC)
        ...
        ELSE(NOT CMAKE24)
            ...
            IF(OpenPointCloud_MSVC_VERSIONED_DLL) 
            ...
            ELSE(OpenPointCloud_MSVC_VERSIONED_DLL)
            ...
            ENDIF(OpenPointCloud_MSVC_VERSIONED_DLL)
...
MACRO(SETUP_EXE IS_COMMANDLINE_APP)
    ...
    ELSE(${IS_COMMANDLINE_APP})
        ...
        IF(APPLE)
            # SET(MACOSX_BUNDLE_LONG_VERSION_STRING "${OpenPointCloud_MAJOR_VERSION}.${OpenPointCloud_MINOR_VERSION}.${OpenPointCloud_PATCH_VERSION}")
            # Short Version is the "marketing version". It is the version
            # the user sees in an information panel.
            SET(MACOSX_BUNDLE_SHORT_VERSION_STRING "${OpenPointCloud_MAJOR_VERSION}.${OpenPointCloud_MINOR_VERSION}.${OpenPointCloud_PATCH_VERSION}")
            # Bundle version is the version the OS looks at.
            SET(MACOSX_BUNDLE_BUNDLE_VERSION "${OpenPointCloud_MAJOR_VERSION}.${OpenPointCloud_MINOR_VERSION}.${OpenPointCloud_PATCH_VERSION}")
    ...
    IF(MSVC_IDE AND OpenPointCloud_MSVC_VERSIONED_DLL)
        SET_OUTPUT_DIR_PROPERTY_260(${TARGET_TARGETNAME} "")        # Ensure the /Debug /Release are removed
    ENDIF(MSVC_IDE AND OpenPointCloud_MSVC_VERSIONED_DLL)
...
MACRO(HANDLE_MSVC_DLL)
        ...
        #it is hardcoded, we should compute OUTPUT_BINDIR position relative to OUTPUT_LIBDIR ... to be implemented
        #changing bin to something else breaks this hack
        #the dll are versioned by prefixing the name with DCViz${OpenPointCloud_SOVERSION}-
        ...
        ELSE(${ARGC} GREATER 1)
                SET(LIB_SOVERSION ${OpenPointCloud_SOVERSION})
        ENDIF(${ARGC} GREATER 1)
...
#     SET_TARGET_PROPERTIES(${LIB_NAME} PROPERTIES PREFIX "../../bin/DCViz${OpenPointCloud_SOVERSION}-")
