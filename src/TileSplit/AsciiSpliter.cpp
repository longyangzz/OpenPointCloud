		...
		for (auto it = fMapUp.begin(); it != fMapUp.end(); ++it)
		{
			...
			for (int fnum = 0; fnum != 4; ++fnum)
			{
				...
				while(inFile.readLine(currentLine, 500) > 0)
				{
					...
					Point3D currentPoint = Point3D(boxlist[0].toDouble(), boxlist[1].toDouble(), boxlist[2].toDouble());
...
BoundBox AsciiSpliter::GenSubBoxByFileName(const QString& name)
{
	...
	Point_3D minCorner = m_boundBox.m_minCorner + Point_3D(m_boundBox.m_length / float(cr) * xIndex, m_boundBox.m_width / float(cr) * yIndex, 0 );
	Point_3D maxCorner = m_boundBox.m_minCorner + Point_3D(m_boundBox.m_length / float(cr) * (xIndex + 1), m_boundBox.m_width / float(cr) * (yIndex+1), (m_boundBox.m_maxCorner.z - m_boundBox.m_minCorner.z));
