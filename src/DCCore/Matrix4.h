...
namespace DCCore
{
   ...
   * \sa Vector4D, Vector3D, Vector2D, Matrix3, Matrix2
   */
  ...
  public:
    ...
    //-----------------------------------------------------------------------------
    Vector3D<T_Scalar> getX() const
    {
      return Vector3D<T_Scalar>(mVec[0].x, mVec[0].y, mVec[0].z);
    }
    //-----------------------------------------------------------------------------
    Vector3D<T_Scalar> getY() const
    {
      return Vector3D<T_Scalar>(mVec[1].x, mVec[1].y, mVec[1].z);
    }
    //-----------------------------------------------------------------------------
    Vector3D<T_Scalar> getZ() const
    {
      return Vector3D<T_Scalar>(mVec[2].x, mVec[2].y, mVec[2].z);
    }
    //-----------------------------------------------------------------------------
    Vector3D<T_Scalar> getT() const
    {
      return Vector3D<T_Scalar>(mVec[3].x, mVec[3].y, mVec[3].z);
    }
    //-----------------------------------------------------------------------------
    Matrix4& setX(const Vector3D<T_Scalar>& v) 
    {
      mVec[0].x = v.x;
      mVec[0].y = v.y;
      mVec[0].z = v.z;
    ...
    //-----------------------------------------------------------------------------
    Matrix4& setY(const Vector3D<T_Scalar>& v) 
    {
      mVec[1].x = v.x;
      mVec[1].y = v.y;
      mVec[1].z = v.z;
    ...
    //-----------------------------------------------------------------------------
    Matrix4& setZ(const Vector3D<T_Scalar>& v) 
    {
      mVec[2].x = v.x;
      mVec[2].y = v.y;
      mVec[2].z = v.z;
    ...
    //-----------------------------------------------------------------------------
    Matrix4& setT(const Vector3D<T_Scalar>& v) 
    {
      mVec[3].x = v.x;
      mVec[3].y = v.y;
      mVec[3].z = v.z;
    ...
    //-----------------------------------------------------------------------------
    static Matrix4 getLookAtModeling(const Vector3D<T_Scalar>& eye, const Vector3D<T_Scalar>& at, const Vector3D<T_Scalar>& up);
    //-----------------------------------------------------------------------------
    static Matrix4 getLookAt(const Vector3D<T_Scalar>& eye, const Vector3D<T_Scalar>& at, const Vector3D<T_Scalar>& up);
    //-----------------------------------------------------------------------------
    void getAsLookAtModeling(Vector3D<T_Scalar>& eye, Vector3D<T_Scalar>& at, Vector3D<T_Scalar>& up, Vector3D<T_Scalar>& right) const;
    //-----------------------------------------------------------------------------
    void getAsLookAt(Vector3D<T_Scalar>& eye, Vector3D<T_Scalar>& at, Vector3D<T_Scalar>& up, Vector3D<T_Scalar>& right) const;
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    static Matrix4 getRotation(T_Scalar degrees, const Vector3D<T_Scalar>& v)
    {
      return getRotation(degrees, v.x, v.y, v.z);
    }
    //-----------------------------------------------------------------------------
    static Matrix4 getRotation(T_Scalar degrees1, const Vector3D<T_Scalar>& v1, T_Scalar degrees2, const Vector3D<T_Scalar>& v2)
    {
      return getRotation(degrees1, v1.x, v1.y, v1.z) * getRotation(degrees2, v2.x, v2.y, v2.z);
    }
    //-----------------------------------------------------------------------------
    static Matrix4 getRotation(T_Scalar degrees1, const Vector3D<T_Scalar>& v1, T_Scalar degrees2, const Vector3D<T_Scalar>& v2, T_Scalar degrees3, const Vector3D<T_Scalar>& v3)
    {
      return getRotation(degrees1, v1.x, v1.y, v1.z) * getRotation(degrees2, v2.x, v2.y, v2.z) * getRotation(degrees3, v3.x, v3.y, v3.z);
    }
    //-----------------------------------------------------------------------------
    Matrix4& rotate(T_Scalar degrees, const Vector3D<T_Scalar>& v)
    {
    ...
    //-----------------------------------------------------------------------------
    Matrix4& rotate(T_Scalar degrees1, const Vector3D<T_Scalar>& v1, T_Scalar degrees2, const Vector3D<T_Scalar>& v2)
    {
      return preMultiply(getRotation(degrees1, v1, degrees2, v2));
    }
    //-----------------------------------------------------------------------------
    Matrix4& rotate(T_Scalar degrees1, const Vector3D<T_Scalar>& v1, T_Scalar degrees2, const Vector3D<T_Scalar>& v2, T_Scalar degrees3, const Vector3D<T_Scalar>& v3)
    {
    ...
    //-----------------------------------------------------------------------------
    static Matrix4& getRotation(Matrix4& out, const Vector3D<T_Scalar>& from, const Vector3D<T_Scalar>& to);
    //-----------------------------------------------------------------------------
    static Matrix4 getRotation(const Vector3D<T_Scalar>& from, const Vector3D<T_Scalar>& to)
    {
      Matrix4 m;
      return getRotation(m, from, to);
    }
    //-----------------------------------------------------------------------------
    Matrix4& rotate(const Vector4D<T_Scalar>& from, const Vector4D<T_Scalar>& to)
    {
      return preMultiply(getRotation(from, to));
    }
    //-----------------------------------------------------------------------------
    Matrix4& rotate(const Vector3D<T_Scalar>& from, const Vector3D<T_Scalar>& to)
    {
      return preMultiply(getRotation(from, to));
    }
    //-----------------------------------------------------------------------------
    static Matrix4& getTranslation(Matrix4&out, const Vector3D<T_Scalar>& v)
    {
      return getTranslation(out, v.x, v.y, v.z);
    }
    //-----------------------------------------------------------------------------
    static Matrix4 getTranslation(const Vector3D<T_Scalar>& v)
    {
      Matrix4 m;
    ...
    //-----------------------------------------------------------------------------
    Matrix4& translate(const Vector3D<T_Scalar>& v)
    {
      return preMultiply(getTranslation(v));
    }
    //-----------------------------------------------------------------------------
    static Matrix4& getScaling(Matrix4& out, const Vector3D<T_Scalar>& v)
    {
      return getScaling(out, v.x, v.y, v.z);
    }
    //-----------------------------------------------------------------------------
    static Matrix4 getScaling(const Vector3D<T_Scalar>& v)
    {
      Matrix4 m;
    ...
    //-----------------------------------------------------------------------------
    Matrix4& scale(const Vector3D<T_Scalar> v)
    {
  ...
  private:
    const Vector4D<T_Scalar>& operator[](unsigned int i) const { return mVec[i]; }
    Vector4D<T_Scalar>& operator[](unsigned int i) { return mVec[i]; }

  protected:
    Vector4D<T_Scalar> mVec[4];
  };
  //-----------------------------------------------------------------------------
  // OPERATORS
  //-----------------------------------------------------------------------------
  ...
  template<typename T_Scalar>
  inline Vector4D<T_Scalar> operator*(const Matrix4<T_Scalar>& m, const Vector4D<T_Scalar>& v)
  {
    return Vector4D<T_Scalar>(
  ...
  //-----------------------------------------------------------------------------
  //! Post multiplication: matrix * column vector
  //! The incoming vector is considered a Vector4D<T_Scalar> with the component w = 1
  template<typename T_Scalar>
  inline Vector3D<T_Scalar> operator*(const Matrix4<T_Scalar>& m, const Vector3D<T_Scalar>& v)
  {
    return Vector3D<T_Scalar>(
  ...
  //-----------------------------------------------------------------------------
  //! Post multiplication: matrix * column vector
  //! The incoming vector is considered a Vector4D<T_Scalar> with components: z = 0 and w = 1
  template<typename T_Scalar>
  inline Vector2D<T_Scalar> operator*(const Matrix4<T_Scalar>& m, const Vector2D<T_Scalar>& v)
  {
    return Vector2D<T_Scalar>(
  ...
  template<typename T_Scalar>
  inline Vector4D<T_Scalar> operator*(const Vector4D<T_Scalar>& v, const Matrix4<T_Scalar>& m)
  {
    return Vector4D<T_Scalar>(
  ...
  //! The incoming vector is considered a Vector4D<T_Scalar> with the component w = 1
  template<typename T_Scalar>
  inline Vector3D<T_Scalar> operator*(const Vector3D<T_Scalar>& v, const Matrix4<T_Scalar>& m)
  {
    return Vector3D<T_Scalar>(
  ...
  //! The incoming vector is considered a Vector4D<T_Scalar> with components: z = 0 and w = 1
  template<typename T_Scalar>
  inline Vector2D<T_Scalar> operator*(const Vector2D<T_Scalar>& v, const Matrix4<T_Scalar>& m)
  {
    return Vector2D<T_Scalar>(
  ...
  //-----------------------------------------------------------------------------
  template<typename T_Scalar>
  Matrix4<T_Scalar> Matrix4<T_Scalar>::getLookAtModeling(const Vector3D<T_Scalar>& eye, const Vector3D<T_Scalar>& at, const Vector3D<T_Scalar>& up)
  {
    Vector3D<T_Scalar> zaxis = (eye-at).normalize();
    Vector3D<T_Scalar> xaxis = cross(up, zaxis).normalize();
    Vector3D<T_Scalar> yaxis = cross(zaxis, xaxis);
  ...
  //-----------------------------------------------------------------------------
  template<typename T_Scalar>
  Matrix4<T_Scalar> Matrix4<T_Scalar>::getLookAt(const Vector3D<T_Scalar>& eye, const Vector3D<T_Scalar>& at, const Vector3D<T_Scalar>& up)
  {
    Vector3D<T_Scalar> zaxis = (eye-at).normalize();
    Vector3D<T_Scalar> xaxis = cross(up, zaxis).normalize();
    Vector3D<T_Scalar> yaxis = cross(zaxis, xaxis);
  ...
  //-----------------------------------------------------------------------------
  template<typename T_Scalar>
  void Matrix4<T_Scalar>::getAsLookAtModeling(Vector3D<T_Scalar>& eye, Vector3D<T_Scalar>& at, Vector3D<T_Scalar>& up, Vector3D<T_Scalar>& right) const
  {
    eye = getT();
  ...
  //-----------------------------------------------------------------------------
  template<typename T_Scalar>
  void Matrix4<T_Scalar>::getAsLookAt(Vector3D<T_Scalar>& eye, Vector3D<T_Scalar>& at, Vector3D<T_Scalar>& up, Vector3D<T_Scalar>& right) const
  {
  ...
  Matrix4<T_Scalar> Matrix4<T_Scalar>::getPerspective(T_Scalar fovy, T_Scalar aspect_ratio, T_Scalar znear, T_Scalar zfar)
  {
    ...
    T_Scalar rads = (fovy / ((T_Scalar)2)) * (T_Scalar)DC_DEG_TO_RAD;
  ...
  Matrix4<T_Scalar>& Matrix4<T_Scalar>::getRotation(Matrix4<T_Scalar>& out, T_Scalar degrees, T_Scalar x, T_Scalar y, T_Scalar z)
  {
    ...
    degrees = T_Scalar(degrees * DC_DEG_TO_RAD);
  ...
  //-----------------------------------------------------------------------------
  template<typename T_Scalar>
  Matrix4<T_Scalar>& Matrix4<T_Scalar>::getRotation(Matrix4<T_Scalar>& out, const Vector3D<T_Scalar>& from, const Vector3D<T_Scalar>& to)
  {
    Vector3D<T_Scalar> a,b;
    a = from;
    b = to;
    a.normalize();
    b.normalize();
    ...
    Vector3D<T_Scalar> axis,n2;
    axis = cross(a,b);
    axis.normalize();
    T_Scalar alpha = acos(cosa);
    return getRotation(out, alpha*(T_Scalar)DC_RAD_TO_DEG, axis.x, axis.y, axis.z);
  }
  //-----------------------------------------------------------------------------
  ...
  {
    Vector3D<T_Scalar> vx = getX();
    Vector3D<T_Scalar> vy = getY();
    Vector3D<T_Scalar> vz = getZ();

    vx.normalize();
    vy.normalize();
    vz.normalize();

    T_Scalar kx = dot(vy,Vector3D<T_Scalar>(0,1,0));
    kx = clamp(kx,-((T_Scalar)1),+((T_Scalar)1));
    degrees_x = acos(kx) * (T_Scalar)DC_RAD_TO_DEG;
    if(dot(vz, Vector3D<T_Scalar>(0,1,0)) > 0)
      degrees_x = -degrees_x;

    T_Scalar ky = dot(vx, Vector3D<T_Scalar>(1,0,0));
    ky = clamp(ky,-((T_Scalar)1),+((T_Scalar)1));
    degrees_y = acos(ky) * (T_Scalar)DC_RAD_TO_DEG;
    if(dot(vz, Vector3D<T_Scalar>(1,0,0)) < 0)
      degrees_y = -degrees_y;
